...
local 직접 실행과 독립 풀 데모에서 Spring Boot 기동 후 스키마가 준비된 상태에 수동으로 한 번 실행합니다. prod 설치는 이 파일을 실행하지 않고 Flyway의 공장·구역·device·채널 토폴로지를 사용하며 계정은 별도로 준비합니다.
...
> 이 파일의 알려진 비밀번호는 로컬 시연용이며 운영 DB에 투입하지 않습니다.
