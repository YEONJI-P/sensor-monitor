...
사번(employeeId) 기반의 승인제 회원 관리와 5단계 역할 기반 접근 제어(RBAC)를 통해, 전체 시스템·공장·구역 단위로 접근 범위를 제한합니다.
...
| GET | `/zones` | 접근 가능한 구역 조회 (SYSTEM_ADMIN·SYSTEM_VIEWER 전체, FACTORY_ADMIN 소속 공장, MEMBER·VIEWER 배정 구역) | JWT |
...
> EventSource가 헤더를 못 실어 Access Token을 쿼리로 받습니다. 구독자는 자신의 접근 가능 장치로 이벤트가 필터링됩니다. `SYSTEM_ADMIN`·`SYSTEM_VIEWER`는 전체, `FACTORY_ADMIN`은 소속 공장, `MEMBER`·`VIEWER`는 배정 구역의 telemetry를 조회합니다. `sensor-data` 이벤트는 batch 단위 판독과 `stateApplied`를, `alert` 이벤트는 실제 notification을, `alarm-episode` 이벤트는 `OPEN`·`RESOLVED`·`ACK`·`ENRICHED` 상태 변경을 전달합니다. 모두 DB 트랜잭션 커밋 후 전송하므로 rollback된 유령 이벤트는 없습니다. 연결이 닫히면 refresh token으로 access token을 갱신해 재구독하고, 30초 polling으로 인프로세스 SSE 유실을 보정합니다.
...
- 5단계 역할 기반 접근 제어
  ...
  | `SYSTEM_VIEWER` | 전체 공장·구역의 운영 데이터 읽기 전용. 사용자 관리·설정 변경·알람 확인 처리 불가 |
...
- 5단계 역할 기반 접근 제어, 전체 시스템·공장·구역 계층 접근 제어
...
> 컨테이너 postgres는 호스트 `5433`, backend는 `8080`, explain은 `8000`에 노출됩니다. backend는 내부 네트워크의 `postgres:5432`를 사용하므로 서비스 env의 `DB_*` 값보다 Compose 토폴로지 값이 우선합니다. `SYSTEM_ADMIN` 계정 `SYSTEM`과 `SYSTEM_VIEWER` 계정 `DEMO`는 위 수동 seed 명령이 만들며 Compose 기동이나 Flyway가 만들지 않습니다. `docker compose down`은 volume을 유지하고, `down -v`는 데모 DB를 삭제하므로 데이터 삭제 의도가 있을 때만 사용합니다.
...
local 직접 실행과 독립 풀 데모에서 Spring Boot 기동 후 스키마가 준비된 상태에 수동으로 한 번 실행합니다. prod와 홈서버 설치는 이 파일을 실행하지 않고 Flyway의 공장·구역·device·채널 토폴로지를 사용하며, 운영 관리자와 단일 `SYSTEM_VIEWER` 데모 계정 bootstrap은 별도로 해결해야 합니다.
...
- V10(`V10__add_system_viewer_role.sql`)은 사용자 역할 CHECK 제약에 전체 운영 데이터 읽기 전용 `SYSTEM_VIEWER`를 추가합니다.
- V2~V10 모두 사용자, 구역 소속, 비밀번호를 만들지 않습니다. 따라서 공개 서버의 첫 관리자와 단일 `SYSTEM_VIEWER` 데모 계정 bootstrap 절차는 배포 전에 별도로 확정해야 합니다.
...
공장(Factory), 구역(Zone), 구역 소속(ZoneUser) 3계층으로 접근 범위를 계산합니다. `SYSTEM_ADMIN`과 `SYSTEM_VIEWER`는 전체, `FACTORY_ADMIN`은 소속 공장, `MEMBER`와 `VIEWER`는 소속 구역으로 범위가 좁혀집니다. `SYSTEM_VIEWER`와 `VIEWER`는 읽기 전용이며 장치·채널·구역 변경과 알람 확인 처리가 차단됩니다. `SYSTEM_VIEWER`는 공개 데모용 단일 계정으로만 bootstrap하며 일반 가입 승인에서는 부여하지 않습니다.
