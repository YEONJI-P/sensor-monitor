...
gh pr create \
  ...
  --body-file - <<EOF
Sensor Monitor main CI와 backend·explain·simulator 이미지 발행이 모두 성공한 뒤 자동으로 생성된 배포 제안입니다.

- source SHA: \`$SOURCE_SHA\`
- workflow run: $GITHUB_SERVER_URL/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID
- 변경 범위: \`docker-compose.yml\`의 Sensor backend·explain·simulator image pin 세 줄만
- 자동 적용: 병합 후 bugi-server-infra timer의 Sensor image-only fast-forward gate 대상

병합 전 확인:

- [ ] 세 GHCR 이미지가 모두 source SHA로 발행됨
- [ ] 세 image pin이 같은 40자리 SHA임
